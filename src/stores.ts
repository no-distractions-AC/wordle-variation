...
class Writable<T> {
  ...
  public subscribe(
    run: (val: T) => void,
    invalidate: (val?: T) => void = Writable.noop,
  ) {
...
export const activeVariation = writable<string>(
  localStorage.getItem("activeVariation") || "classic",
);
activeVariation.subscribe((v) => localStorage.setItem("activeVariation", v));
