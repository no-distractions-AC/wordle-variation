...
export const words = createWordList(wordList);

export function createWordList(list: { words: string[]; valid: string[] }) {
  return {
    ...list,
    contains: (word: string) => {
      return list.words.includes(word) || list.valid.includes(word);
    },
  };
}
...
export function getRowData(n: number, board: GameBoard, cols: number = COLS) {
  const wd = new WordData(cols);
...
export class GameState extends Storable {
  ...
  constructor(
    ...
    if (!this.#valid) {
      ...
      this.board = {
        words: Array(this.rows).fill(""),
        state: Array.from({ length: this.rows }, () =>
  ...
  guess(word: string) {
    ...
    const result = Array<LetterState>(this.cols).fill("⬛");
    for (let i = 0; i < this.cols; ++i) {
